...
public sealed partial class MediaPlaybackSection : UserControl
{
    ...
    private bool ShowArtistsContextMenuOnButton(UIElement nativeElement)
    {
        // --- Source data & guards ---
        ...
        foreach (var artistName in namesList)
        {
            ...
            viewAlbums.RightTapped += (s, e) => TryVM(a => a.QuickViewArtistSortByAlbum());

            var viewGenres = new Microsoft.UI.Xaml.Controls.MenuFlyoutItem { Text = "Genres" };
            viewGenres.Click += (_, __) => TryVM(a => a.NavigateToArtistPage(song, artistName)); 
            viewGenres.RightTapped += (s, e) => TryVM(a => a.QuickViewArtistSortByGenres());
            ...
            var lastMFI = new Microsoft.UI.Xaml.Controls.MenuFlyoutItem { Text = $"View Full Artist Stats:" };
            lastMFI.Click += (s, e) =>
            {
                MyViewModel.NavigateToArtistPage(song, artistName);
            };
            stats.Items.Add(lastMFI);
    ...
    private void ArtistBtn_Click(object sender, RoutedEventArgs e)
    {
        var nativeElement = (Microsoft.UI.Xaml.UIElement)sender;
            ...
            bool flowControl = ShowArtistsContextMenuOnButton(nativeElement );
            if (!flowControl)
            {
                return;
            }
    ...
    private void ArtistBtn_RightTapped(object sender, RightTappedRoutedEventArgs e)
    {
        var nativeElement = (Microsoft.UI.Xaml.UIElement)sender;
