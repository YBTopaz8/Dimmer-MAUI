...
[Service(Name = "com.yvanbrunel.dimmer.QuickSettingsTileService", // <<< CHANGE TO YOUR UNIQUE NAME
           ...
           Icon = "@drawable/exo_icon_circular_play", // Create this drawable icon
    ...
    public void UpdateTileVisualState(bool isPlaying, SongModelView? song = null)
    {
        ...
        try
        {
            ...
            if (isPlaying)
            {
                ...
                newIcon = Icon.CreateWithResource(Platform.AppContext, global::Dimmer.Resource.Drawable.media3_icon_circular_play); // <<< CREATE THIS ICON
            }
            else
            {
                newIcon = Icon.CreateWithResource(Platform.AppContext, Resource.Drawable.media3_icon_circular_play); // <<< CREATE THIS ICON
