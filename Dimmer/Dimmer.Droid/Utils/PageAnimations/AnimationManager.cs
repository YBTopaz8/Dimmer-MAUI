...
public static class AnimationManager
{
    ...
    public static PageAnimationProfile GetPageAnimations(Type pageType, IAnimationService animationService)
    {
        ...
        string? pushEnterId = Preferences.Get($"{pageIdentifier}_PushEnterAnim", animationService.GetDefaultPushEnterAnimation().ResourceId.ToString());
        profile.PushEnter = availableAnims.FirstOrDefault(a => a.ResourceId.ToString() == pushEnterId) ?? animationService.GetDefaultPushEnterAnimation();

        string? pushExitId = Preferences.Get($"{pageIdentifier}_PushExitAnim", animationService.GetDefaultPushExitAnimation().ResourceId.ToString());
        profile.PushExit = availableAnims.FirstOrDefault(a => a.ResourceId.ToString() == pushExitId) ?? animationService.GetDefaultPushExitAnimation();

        string? popEnterId = Preferences.Get($"{pageIdentifier}_PopEnterAnim", animationService.GetDefaultPopEnterAnimation().ResourceId.ToString());
        profile.PopEnter = availableAnims.FirstOrDefault(a => a.ResourceId.ToString() == popEnterId) ?? animationService.GetDefaultPopEnterAnimation();

        string? popExitId = Preferences.Get($"{pageIdentifier}_PopExitAnim", animationService.GetDefaultPopExitAnimation().ResourceId.ToString());
        profile.PopExit = availableAnims.FirstOrDefault(a => a.ResourceId.ToString() == popExitId) ?? animationService.GetDefaultPopExitAnimation();
