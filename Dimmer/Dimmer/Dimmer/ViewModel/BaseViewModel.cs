...
public partial class BaseViewModel : ObservableObject, IReactiveObject, IDisposable
{
    ...
    public async Task SanitizeSkippedEventsAsync()
    {
        ...
        try
        {
            ...
            if (allSkippedEvents is null || !allSkippedEvents.Any())
            {
    ...
    private async void OnCurrentSongChanged(SongModelView? songView)
    {
        if (songView is null)
            return;
        ...
        CurrentPlayingSongView.CurrentPlaySongDominantColor = await ImageResizer.GetDomminantMauiColorAsync(CurrentPlayingSongView.CoverImagePath);
        CurrentPlaySongDominantColor = CurrentPlayingSongView.CurrentPlaySongDominantColor;
