       ...
       await _realm.WriteAsync(() =>
        {
            ...
            foreach (var song in allSongs)
            {
                ...
                song.HasSyncedLyrics = song.SyncLyrics?.Length > 0; // Simplified ternary

                // 2. Update Aggregated Notes
                if (song.UserNotes?.Any() == true)
                {
            ...
            foreach (var album in _realm.All<AlbumModel>())
            {
            ...
            foreach (var artist in _realm.All<ArtistModel>())
            {
            ...
            int rank = 1;
            var sortedSongs = _realm.All<SongModel>().AsEnumerable().OrderByDescending(s => s.EngagementScore > 0 ? s.EngagementScore : s.PopularityScore); // Fallback to PopularityScore
            ...
            rank = 1;
            var sortedAlbums = _realm.All<AlbumModel>().OrderByDescending(a => a.TotalCompletedPlays);
            ...
            rank = 1;
            var sortedArtists = _realm.All<ArtistModel>().OrderByDescending(a => a.TotalCompletedPlays);
