...
public static class NaturalLanguageProcessor
{
    ...
    public static string Process(string naturalQuery)
    {
        ...
        // FIX: The lookbehind (?<!\b(?:asc|desc|by|first|last|random|shuffle)) ensures that fields following 
        // directive keywords are NOT inappropriately tagged with "any:". Variable-width lookbehinds are supported in .NET.
        processedQuery = Regex.Replace(
            processedQuery,
            @"(?<!\b(?:asc|desc|by|first|last|random|shuffle))\s+(?!and|or|not|add|include|exclude|remove|asc|desc|shuffle|random|first|last|by|[a-zA-Z0-9_]+:)(\w+)",
            " any:$1",
