...
public partial class PlaybackUtilsService : ObservableObject, IPlaybackUtilsService
{
        ...
        bool IsFromPreviousOrNext = false, AppState CurrentAppStatee = AppState.OnForeGround)
    {
        ...
        try
        {
            ...
            if (song != null)
            {
                if (!File.Exists(song.FilePath))
                {
                    ...
                    if (await SongsMgtService.DeleteSongFromDB(song.Id))
                    {
                        chckA = "X";
                    }
            ...
            chckA = "M";            // Add current time with 'false' for incomplete play
        ...
        catch (Exception e)
        {
            ...
            await Shell.Current.DisplayAlert("Error message!!!!", $"********** UNHANDLED EXCEPTION! Details: {chckA} {e.Message} | {e.Source} " +
            $"| {e.TargetSite} ", "Ok");
            return false;
        }
        finally
        ...
        try
        {
            ...
            if (IsNext)
            {
                ...
                if (isTrueShuffle)
                {
                    ...
                    while (newPosition == currentPosition && nowPlayingShuffledOrNot.Count > 1)
                    {
                        newPosition = random.Next(0, nowPlayingShuffledOrNot.Count);
                    }
