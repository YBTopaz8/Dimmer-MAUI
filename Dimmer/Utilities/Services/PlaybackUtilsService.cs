...
public partial class PlaybackUtilsService : ObservableObject, IPlaybackUtilsService
{
    ...
    {
        await SongsMgtService.LoadSongsFromFolderAsync(folderPaths, false);
    ...
    public bool PlaySelectedSongsOutsideApp(List<string> filePaths)
    {
        ...
        if (allSongs.Count > 0)
        {
            ...
            PlaySong(ObservableCurrentlyPlayingSong, 2, allSongs);
        }
    ...
    public bool PlaySong(SongModelView? song, bool isPreview = true)
    {
        ...
        if (song is null)
        {
            _ = Shell.Current.DisplayAlert("Info", "Unable To Preview For Now...", "OK");
            return false;
        }
        ...
        audioService.Play();
        audioService.SetCurrentTime(sixtyPercentOfSong);  
        CurrentQueue = 2;
    ...
    bool isLoadingSongs;
    public bool PlaySong(SongModelView? song = null, int currentQueue = 0,
        ...
        bool IsFromPreviousOrNext = false, AppState CurrentAppStatee = AppState.OnForeGround)
    {
        ...
        else
        {
            if (song is null)
            {
                ...
                if (ObservableCurrentlyPlayingSong is null)
                {
                    if (!isLoadingSongs)
                    {
                        _ = Shell.Current.DisplayAlert("Error when trying to play song", "Song Not Ready to play yet!", "OK");
                        return false;
                    }
                }
            }
        ...
        try
        {
            ...
            ViewModel.Value.CurrPL.Clear();
            ViewModel.Value.CurrPL.Add(song);
            if (currentList is null)
            {
                currentList = SongsMgtService.AllSongs.Take(25).ToObservableCollection();
            }
            foreach (var item in currentList)
            {
                ViewModel.Value.CurrPL.Add(item);
            }
            if (CurrentQueue == 2)
            {
                audioService.Initialize(ObservableCurrentlyPlayingSong);
            ...
            if (positionInSec > 0)
            {
                audioService.Play(IsFromPreviousOrNext);
                currentPositionInSec = positionInSec;
                audioService.SetCurrentTime(currentPositionInSec);
            }
            else
            {
                audioService.Play(true);
            }
        ...
        finally
        {
            ...
            if ((File.Exists(ObservableCurrentlyPlayingSong!.FilePath) && ObservableCurrentlyPlayingSong != null))// && currentQueue != 2)
            {
                ...
                //await SongsMgtService.AddPlayAndCompletionLink(link);
    ...
    {
        //bool isLocked = await _playLock.Wait(0);
        //if (!isLocked)
        //    return;
            ...
            {
                PlaySong(IsFromPreviousOrNext: true, IsUserSkipped:false);
                return;
            }
    ...
    #endregion
    public bool PlayNextSong(bool IsUserSkipped = true)
    {
        if (CurrentQueue == 2)
        {
            return true;
        ...
        return true;
        return PlaySong(ObservableCurrentlyPlayingSong, CurrentQueue, IsFromPreviousOrNext: true);
    }
    ...
    public bool PlayPreviousSong(bool IsUserSkipped = true)
    {
        ...
        audioService.SkipToNext();
        return true;
        return PlaySong(ObservableCurrentlyPlayingSong, CurrentQueue, IsUserSkipped:IsUserSkipped, IsFromPreviousOrNext: true);
    }
    ...
    private void OnPositionTimerElapsed(object? sender, ElapsedEventArgs e)
    {
        ...
        //{
        //   _ = LastfmClient.Instance.Track.UpdateNowPlaying(
